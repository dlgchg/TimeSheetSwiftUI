...
struct ToDoView: View {
    ...
    var body: some View {
        Section {
            if items.count > 0 {
                ForEach(items.filter({searchKey.isEmpty ? true : ($0.content!.contains(searchKey) ||  $0.important == (searchKey.contains("重要") ? true : false))})) { item in
...
struct ToDoView_Previews: PreviewProvider {
    static var previews: some View {
        ToDoView(sheet: .constant(nil), searchKey: .constant(""))
    }
}
