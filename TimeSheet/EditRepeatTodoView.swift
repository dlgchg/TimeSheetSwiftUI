//
//  EditRepeatTodoView.swift
//  TimeSheet
//
//  Created by 李伟 on 2020/12/3.
//

import SwiftUI
import CoreData

struct EditRepeatTodoView: View {
    ...
    @State var repeatItem = RepeatItem(context: context)
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                .navigationBarTitle("重复编辑", displayMode: .inline)
                .navigationBarBackButtonHidden(true)
                .navigationBarItems(leading: Button("取消", action: {
                    shock()
                    self.presentationMode.wrappedValue.dismiss()
                }).foregroundColor(.primary), trailing: Button("更新", action: {
                    shock()
            ...
            .onAppear {
                ...
                let request = NSFetchRequest<RepeatItem>(entityName: "RepeatItem")
                request.fetchLimit = 1
                request.predicate = NSPredicate(format: "id == %@", repeatId as CVarArg)
                do {
                    ...
                    if items.count > 0 {
                        ...
                        postpone = repeatItem.postpone
                        allday = repeatItem.allday
                        sound = repeatItem.sound!
                        showLunar = repeatItem.type == 5
                        repeatType = Int(repeatItem.type)
                        if repeatType == 2 {
                            weekType = repeatItem.ttype!
                        }
                        if repeatType == 3 {
                            monthType = Int(repeatItem.ttype!.substring(to: 1))!
                        }
                        if postpone && allday {
                            todo = true
                        }
                        if repeatItem.lunarCalendar != nil && !repeatItem.lunarCalendar!.isEmpty {
                            lunarCalendar = repeatItem.lunarCalendar!
                            
                        }
                        content = repeatItem.content!
                        if repeatItem.categoryName != nil &&  !repeatItem.categoryName!.isEmpty {
                            categoryType = categoryItems.firstIndex(where: {repeatItem.categoryName! == $0.name!})!
                        }
                    }
                } catch {
                    print(error.localizedDescription)
...
struct EditRepeatTodoView_Previews: PreviewProvider {
