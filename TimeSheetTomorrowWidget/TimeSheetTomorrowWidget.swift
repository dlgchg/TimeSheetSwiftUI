//
//  TimeSheetTomorrowWidget.swift
//  TimeSheetTomorrowWidget
//
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        ...
        request.predicate = NSCompoundPredicate(type: .or, subpredicates: [
            NSCompoundPredicate(type: .and, subpredicates: [
                NSPredicate(format: "date == %@", Date().date().nextDayDate(1) as CVarArg),
...
struct TimeSheetTomorrowWidgetEntryView : View {
...
@main
struct TimeSheetTomorrowWidget: Widget {
    let kind: String = "时间表"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: Provider()) { entry in
            TimeSheetTomorrowWidgetEntryView(entry: entry)
        }
        .supportedFamilies([.systemMedium, .systemLarge])
        .configurationDisplayName("时间表日程小组件")
        .description("显示明天的待完成日程事项")
    }
}

struct TimeSheetTomorrowWidget_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            TimeSheetTomorrowWidgetEntryView(entry: SimpleEntry(date: Date(), items: []))
