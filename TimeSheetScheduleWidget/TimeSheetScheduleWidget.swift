//
//  TimeSheetScheduleWidget.swift
//  TimeSheetScheduleWidget
//
...
struct TimeSheetScheduleWidgetEntryView : View {
...
@main
struct TimeSheetScheduleWidget: Widget {
    let kind: String = "时间表"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: Provider()) { entry in
            TimeSheetScheduleWidgetEntryView(entry: entry)
        }
        .supportedFamilies([.systemMedium, .systemLarge])
        .configurationDisplayName("时间表日程小组件")
        .description("显示今天最近的待完成日程事项")
    }
}

struct TimeSheetScheduleWidget_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            TimeSheetScheduleWidgetEntryView(entry: SimpleEntry(date: Date(), items: []))
